...
#import "PKCustomSwitch.h"
#import <QuartzCore/QuartzCore.h>

#define DEFAULT_DURATION 0.2f

@interface PKCustomSwitch()
{
...
- (void)initCustomSwithData;

@end

@implementation PKCustomSwitch
...
-(void)awakeFromNib
{
    NSArray *nibs = [[NSBundle mainBundle] loadNibNamed:@"PKCustomSwitch" owner:self options:nil];
...
- (void)drawRect:(CGRect)rect
{
    if (!first) {
        ...
        _middleRect = CGRectMake(-2, 0, _onImage.size.width, _onImage.size.height);
        
        CAShapeLayer *maskLayer = [CAShapeLayer layer];
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_customSwitch.bounds
                                                            cornerRadius:_customSwitch.bounds.size.height / 2.0+10];
